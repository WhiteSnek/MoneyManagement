...
import {
  convertCurrency,
  formatAmount,
} from "../../../../utilityFunctions/currencyUtilities";
...
const Popup: React.FC<PopupProps> = ({ id, togglePopup }) => {
  ...
  const handleBuyNow = () => {
    if (!product.isService) {
      ...
      const url =
        product.link ||
        `https://www.amazon.in/s?k=${queryTitle}&crid=35DRX65TVR9MI&sprefix=mobile%2Caps%2C273&ref=nb_sb_noss_2`;
      ...
      <div className="bg-zinc-800 text-zinc-100 rounded-lg p-6 sm:p-8 shadow-3xl max-w-xs sm:max-w-3xl w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          ...
          <div className="flex flex-col items-start gap-2 col-span-2">
            ...
            {!product.isService && (
              <p className="text-md sm:text-lg font-thin  text-left">
                Specifications:{" "}
                <span className="text-zinc-400">
                  {truncateText(product.specifications, 150)}
                </span>
              </p>
            )}
            <p className="text-md sm:text-lg font-light text-zinc-400">
              ...
              <span className="text-zinc-100 font-medium text-xl">
                {currency.symbol}
                {formatAmount(
                  convertCurrency(parseFloat(product.price), currency.code)
                )}
              </span>
            </p>
            {!product.isService && (
            ...
            <p className="text-md sm:text-lg font-light text-zinc-400">
              ...
              <span className="text-zinc-100 font-medium text-xl">
                {currency.symbol}
                {formatAmount(convertCurrency(totalPrice, currency.code))}
              </span>
            </p>
            <Button
              label={product.isService ? "Pay Now" : "Buy Now"}
              icon={ShoppingCartCheckout}
              className="hover:-translate-y-1 bg-zinc-900 hover:bg-zinc-950 w-1/2"
              onClickFunc={handleBuyNow}
            />
            {buy && (
              <Confirm
