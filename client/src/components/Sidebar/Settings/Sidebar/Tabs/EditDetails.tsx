...
const EditDetails: React.FC = () => {
  ...
  const [monthlyIncome, setMonthlyIncome] = useState<number>(user ? parseInt(user.monthlyIncome.toString()) : 0)
    ...
    <div className="px-4">
      ...
      <div className="p-2">
        ...
        {otpLoading ?<CircularLoader size={24} /> :<Button
            onClickFunc={sendOtp}
            label="Send OTP"
            className={` bg-zinc-900 w-full ${details.email === '' ? 'opacity-50 cursor-not-allowed' :"hover:-translate-y-1 hover:bg-zinc-950"}`}
        ...
        <div className="grid grid-cols-3 gap-2 py-2">
          ...
          <div className="col-span-2">
            <Input value={creditDate} type='text' onChange={(e)=>setCreditDate(e.target.value)} className='w-full'/>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-2 py-2">
          <p className="col-span-1">Change monthly income:</p>
          <div className="col-span-2">
            <NumberInput value={monthlyIncome} onChange={()=>setMonthlyIncome(monthlyIncome)} step={100} />
          </div>
        </div>
      </div>
